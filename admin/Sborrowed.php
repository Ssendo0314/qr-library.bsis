					...
					<div class="box-body table-responsive">
						...
						<table class="table table-bordered table-hover" id="articles">
							...
							<tbody>
								...
								$books = DB::getInstance()->query("SELECT * FROM books WHERE is_Borrowed = 1 AND borrowerID = ?", [$userId]);
								foreach ($books->results() as $books) { ?>
									<tr>
										...
										<td>
											<?php
											// Calculate the due date (dateBorrowed + 3 days)
											$dueDate = strtotime($books->dateBorrowed . ' +3 days');

											// Format the due date as "Month Day, Year" (e.g., "Jan 23, 2001")
											$formattedDueDate = date('M d, Y', $dueDate);

											// Check if the due date is in the past
											if ($dueDate < time()) {
												// If the due date is in the past, set the text color to red
												echo $formattedDueDate;
											} else {
												// If the due date is in the future, keep the default text color
												echo $formattedDueDate;
											}
											?>
										...
										<td align="center">
											<button type="button" class="btn btn-info btn-xs" data-toggle="modal" data-target="#myModal_<?php echo ucwords($books->id); ?>">View QRCODE</button>
											<!-- Modal -->
											<div id="myModal_<?php echo ucwords($books->id); ?>" class="modal fade" role="dialog">
												<div class="modal-dialog">

													<!-- Modal content-->
													<div class="modal-content">
														<div class="modal-header">
															<h4 class="modal-title"><?php echo ucwords($books->bookTitle); ?></h4>
														</div>
														<div class="modal-body">
															<image src="admin/bookQRCodes/<?php echo ucwords($books->qrcode) ?>" />
														</div>
														<div class="modal-footer">
															<a href="admin/bookQRCodes/<?php echo ucwords($books->qrcode) ?>" download><button type="button" class="btn btn-primary">Download</button></a>
															<button type="button" class="btn btn-primary" onclick="printImg('admin/bookQRCodes/<?php echo ucwords($books->qrcode) ?>')">Print</button>
									...
									<div class="modal-body">
										<form enctype="multipart/form-data" method="post" action="addBook.php">
											<label class="control-label" for="isbn">
												<font color="#EC0003">*</font> International Standard Book Number
											</label>
											<div class="form-group">
												<input type="text" class="form-control" id="isbn" name="isbn" placeholder="Input ISBN" required>
											</div>
											<label class="control-label" for="bookTitle">
												<font color="#EC0003">*</font> Book Title
											</label>
											<div class="form-group">
											...
											<label class="control-label" for="datePublished">
												<font color="#EC0003">*</font> Date Published
											</label>
											<div class="form-group">
