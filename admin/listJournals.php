<?php
if (Input::exists()) {
		$PNG_TEMP_DIR =  dirname(__FILE__).DIRECTORY_SEPARATOR.'journalQRCodes'.DIRECTORY_SEPARATOR;
		
		$journals = DB:: getInstance()->get('journals', array('id','=',Input::get('id')));							
		foreach($journals->results() as $journals){
			unlink($PNG_TEMP_DIR.$journals->qrcode);
		}
		
		$contents = DB:: getInstance()->delete('journals', array('id','=',Input::get('id')));													
		Session::flash('Deleted', 'Record has been successfully deleted.');
		Redirect::to('admin.php?action=listJournals');
}
                            ...
                            <div class="box box-primary">
                                ...
                                <div class="box-body table-responsive">
                                    ...
                                    <table class="table table-bordered table-hover" id="articles">
                                        ...
                                        <tbody>
											<?php
												$journals = DB:: getInstance()->query("SELECT * FROM journals");		
												foreach($journals->results() as $journals){ ?>
														<tr>
															<td><?php echo $journals->isbn ; ?></td>
															<td><?php echo $journals->journalTitle ; ?></td>
															<td><?php echo $journals->author ; ?></td>
															<td><?php echo $journals->datePublished ; ?></td>
															<td align="center">
																<?php if($journals->is_borrowed == 1){?>
															...
															<td align="center">
																<button type="button" class="btn btn-info btn-xs" data-toggle="modal" data-target="#myModal_<?php echo ucwords($journals->id); ?>" >View QRCODE</button>
																<!-- Modal -->
																<div id="myModal_<?php echo ucwords($journals->id); ?>" class="modal fade" role="dialog">
																	<div class="modal-dialog">

																	<!-- Modal content-->
																		<div class="modal-content">
																			<div class="modal-header">
																				<h4 class="modal-title"><?php echo ucwords($journals->journalTitle); ?></h4>
																			</div>
																			<div class="modal-body" >
																				<image src="admin/journalQRCodes/<?php echo ucwords($journals->qrcode) ?>"/>
																			</div>
																			<div class="modal-footer">
																				<button type="button" class="btn btn-primary" onclick="printImg('admin/journalQRCodes/<?php echo ucwords($journals->qrcode) ?>')">Print</button>
																				<a href="admin/journalQRCodes/<?php echo ucwords($journals->qrcode) ?>" download><button type="button" class="btn btn-primary">Download</button></a>
																...
																<form method="POST" action="admin.php?action=editJournal&&id=<?php echo $journals->id; ?>" style="display:inline">
								...
								<div class="box-footer">
									...
									<div class="modal-dialog modal-lg" role="document">
										...
										<div class="modal-header">
											...
											<h3 class="modal-title" id="exampleModalLabel">Add New Journal</h3>
										</div>
										<div class="modal-body">
											<form enctype="multipart/form-data" method="post" action="addJournal.php">
												...
												<label class="control-label" for="journalTitle"><font color="#EC0003">*</font> Journal Title</label>
												<div class="form-group">
													<input type="text" class="form-control" id="journalTitle" name="journalTitle" placeholder="Input Title of the Journal" required >
												</div>
