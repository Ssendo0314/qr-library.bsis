...
<body class="skin-blue">
						...
						<div class="box-body">
							<?php if (Session::exists('error')) { ?>
								<div class="alert alert-danger">
									<i class="glyphicon glyphicon-remove"></i> &nbsp;<?php echo Session::flash('error'); ?>
								</div>
							<?php } ?>
							<div class="row">
								<form enctype="multipart/form-data" method="post" action="">
									<div class="row">
										...
										<div class="col-xs-7">
											...
											<div class="row">
												...
												<div class="col-xs-9">
													<div class="form-group">
														...
														<button type="button" class="btn btn-primary" id="checkBorrowerID">Check</button>
													</div>
												</div>
											</div>
											<div class="row">
												<div class="col-xs-2">
													<label class="control-label" for="borrower">Student Name or Borrower's Name</label>
												</div>
												<div class="col-xs-9">
													<div class="form-group">
														<input type="text" class="form-control" id="borrower" name="borrower" readonly>
													</div>
												</div>
											</div>
											...
											<div class="row">
												<div class="col-xs-2">
													<label class="control-label" for="borrowerContact">Borrowers email</label>
												</div>
												<div class="col-xs-9">
													<div class="form-group">
														<input type="text" class="form-control" id="borrowerContact" name="borrowerContact"  readonly/>
													</div>
												</div>
											</div>
											<div class="row">
	...
	<script>
		$(document).ready(function() {
			$('#checkBorrowerID').click(function() {
				var borrowerID = $('#borrowerID').val();

				// Send an AJAX request to fetch the borrower's name
				$.ajax({
					type: 'POST',
					url: 'check_id.php', // Update the URL to the correct path of your PHP script
					dataType: 'json',
					data: {
					...
					success: function(response) {
						if (response.error) {
							...
							$('#borrower').val(response.fullName);
							$('#borrowerContact').val(response.email);
						}
					},
